...
import mentorService, { MentorProfile, MentorStudentSummary, StudentAttendanceAnalytics } from "@/services/mentor.service";
...
export default function FacultyStudentsPage() {
    ...
    useEffect(() => {
        if (selectedStudent?.id) {
            setAnalytics(null);
            mentorService.getStudentAnalytics(parseInt(selectedStudent.id))
                .then(setAnalytics)
    ...
    const activeList = viewMode === "mentees" ? mentees.map(m => ({
        ...
        .filter(s => !mentees.some(m => m.studentId === s.id))
        .map(s => ({ ...s, id: s.id.toString(), semester: s.semester || 4, phone: s.phone || "N/A", avgGrade: s.avgGrade || "N/A" }));
                                                                        ...
                                                                        <div className="grid grid-cols-1 gap-6">
                                                                            ...
                                                                            <div>
                                                                                ...
                                                                                <div className="max-h-40 overflow-y-auto space-y-2">
                                                                                    {analytics.history.slice().reverse().map((entry, i) => (
                                                                                        <div key={i} className="flex justify-between items-center text-sm p-2 bg-muted/50 rounded">
                                                                                            ...
                                                                                            <Badge variant={entry.status === 'PRESENT' ? 'default' : 'destructive'}
                                                                                                className={entry.status === 'PRESENT' ? 'bg-green-100 text-green-700 hover:bg-green-100' : ''}>
                                                                                                {entry.status}
                                                                                            </Badge>
                                                                                        </div>
                                                                                    ))}
